...
struct battleground_strand_of_the_ancients : BattlegroundScript
{
    ...
    void TeleportToEntrancePosition(Player* player) const
    {
        ...
        else if (WorldSafeLocsEntry const* defenderSpawn = sWorldSafeLocsStore.LookupEntry(SPAWN_DEFENDERS))
        {
            if (player->IsInWorld())
                player->TeleportTo(defenderSpawn);
            else
                player->WorldRelocate(defenderSpawn->MapID, defenderSpawn->Loc.X, defenderSpawn->Loc.Y, defenderSpawn->Loc.Z, defenderSpawn->Facing);
        }
    }
