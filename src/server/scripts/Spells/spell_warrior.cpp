...
class spell_warr_charge_effect : public SpellScript
{
    ...
    void HandleCharge(SpellEffIndex /*effIndex*/) const
    {
        ...
        caster->CastSpell(target, SPELL_WARRIOR_CHARGE_SLOW_EFFECT, args);
    }
...
class spell_warr_mortal_strike : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_warr_mortal_strike::HandleMortalWounds, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
// Colossus Smash execute-vulnerability window sooner.
...
// not handled by the absorb aura itself, so they need an explicit script.
class spell_warr_ignore_pain : public SpellScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
        {
            SPELL_WARRIOR_RENEWED_FURY,
            SPELL_WARRIOR_VENGEANCE_FOCUSED_RAGE
        });
    }
...
// 190456 - Ignore Pain (absorb aura)
// The absorb amount scales with how much Rage the warrior spent beyond the base
// cost. Rage is consumed here rather than by the normal resource-cost system
// because the bonus amount spent varies dynamically. The OnAbsorb callback
// applies a 10% bleed-through so the warrior still takes a small hit and isn't
// completely immune while the shield holds.
class spell_warr_ignore_pain_aura : public AuraScript
{
    bool Load() override
    {
        Unit* caster = GetCaster();
        // Record the bonus Rage available above zero (the flat 20 cost was
        // already deducted by the time Load() runs, so whatever remains is
        // the bonus that scales the absorb amount, capped at 400).
    ...
    void CalcAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
    {
        if (Unit* caster = GetCaster())
        {
            // Formula: base AP coefficient * rage fraction (bonus+200 out of 600).
            // 200 represents the minimum contribution even with zero bonus Rage.
            amount = static_cast<int32>(22.3f * caster->GetTotalAttackPowerValue(BASE_ATTACK)
                * (float(m_extraSpellCost + 200) / 600.0f));

            int32 newRage = caster->GetPower(POWER_RAGE) - m_extraSpellCost;
            if (newRage < 0)
                newRage = 0;
    ...
    {
        // 10% bleed-through: the warrior takes a small fraction of each absorbed
        // hit as physical damage so the absorb doesn't feel completely invincible.
        if (Unit* caster = GetCaster())
        {
            SpellNonMeleeDamage spell(caster, caster, GetSpellInfo(), {}, SPELL_SCHOOL_MASK_NORMAL);
            spell.damage = static_cast<uint32>(dmgInfo.GetDamage() * 0.1f);
    ...
    void Register() override
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warr_ignore_pain_aura::CalcAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_warr_ignore_pain_aura::OnAbsorb, EFFECT_0);
    }
