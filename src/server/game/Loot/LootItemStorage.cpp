...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    if (result)
    {
        do
        {
            ...
            lootItem.randomPropertyId = { ItemRandomEnchantmentType(fields[11].GetUInt8()), fields[12].GetUInt32() };
            lootItem.randomSuffix = fields[13].GetUInt32();
            lootItem.context = ItemContext(fields[14].GetUInt8());
            for (std::string_view bonusList : Trinity::Tokenize(fields[15].GetStringView(), ' ', false))
...
bool LootItemStorage::LoadStoredLoot(Item* item, Player* player)
{
    ...
    if (LootTemplate const* lt = LootTemplates_Item.GetLootFor(item->GetEntry()))
    {
        for (auto const& storedItemPair : container->GetLootItems())
        {
            ...
            li.randomPropertyId.Type = storedItemPair.second.RandomPropertyId.Type;
            li.randomPropertyId.Id = storedItemPair.second.RandomPropertyId.Id;
...
void StoredLootContainer::AddLootItem(LootItem const& lootItem, CharacterDatabaseTransaction trans)
{
    ...
    stmt->setUInt8(11, AsUnderlyingType(lootItem.randomPropertyId.Type));
    stmt->setUInt32(12, lootItem.randomPropertyId.Id);
    stmt->setUInt32(13, lootItem.randomSuffix);
    stmt->setUInt8(14, AsUnderlyingType(lootItem.context));
    std::ostringstream bonusListIDs;
    for (int32 bonusListID : lootItem.BonusListIDs)
        bonusListIDs << bonusListID << ' ';
    stmt->setString(15, bonusListIDs.str());
    trans->Append(stmt);
}
