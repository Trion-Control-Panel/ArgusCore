...
bool CriteriaHandler::ModifierSatisfied(ModifierTreeEntry const* modifier, uint64 miscValue1, uint64 miscValue2, WorldObject const* ref, Player* referencePlayer) const
{
    ...
    switch (ModifierTreeType(modifier->Type))
    {
        ...
        {
            uint32 inebriation = std::min(std::max<uint32>(referencePlayer->GetDrunkValue(),  referencePlayer->GetInt32Value(PLAYER_FAKE_INEBRIATION)), 100u);
        ...
        {
            std::vector<uint32> const& characterRestrictions = referencePlayer->GetDynamicValues(PLAYER_DYNAMIC_FIELD_CHARACTER_RESTRICTIONS);
            if (std::find(characterRestrictions.begin(), characterRestrictions.end(), reqValue) == characterRestrictions.end())
                return false;
            break;
        }
        ...
        case ModifierTreeType::GroupMemberCountFromConnectedRealmEqualOrGreaterThan: // 216
        {
            ...
            if (Group const* group = referencePlayer->GetGroup())
                for (GroupReference const& itr : group->GetMembers())
                    if (itr.GetSource() != referencePlayer && referencePlayer->GetUInt32Value(PLAYER_FIELD_VIRTUAL_PLAYER_REALM) == itr.GetSource()->GetUInt32Value(PLAYER_FIELD_VIRTUAL_PLAYER_REALM))
                        ++memberCount;
        ...
        case ModifierTreeType::PlayerHasArtifactPowerRankCountPurchasedEqualOrGreaterThan: // 225
        {
            ...
            if (!artifact)
                return false;
            ItemDynamicFieldArtifactPowers const* artifactPower = artifact->GetArtifactPower(secondaryAsset);
            if (!artifactPower)
                return false;
        ...
        {
            Item* mainHand = referencePlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
            uint32 itemSubclass = ITEM_SUBCLASS_WEAPON_FIST_WEAPON;
            if (mainHand)
            {
                if (mainHand->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON)
                {
                    itemSubclass = mainHand->GetTemplate()->GetSubClass();

                    uint32 visibleItemId = mainHand->GetVisibleEntry(referencePlayer);
                    uint32 itemAppearanceId = mainHand->GetVisibleAppearanceModId(referencePlayer);
                    if (ItemModifiedAppearanceEntry const* itemModifiedAppearance = sDB2Manager.GetItemModifiedAppearance(visibleItemId, itemAppearanceId))
        ...
        {
            Item* offHand = referencePlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
            uint32 itemSubclass = ITEM_SUBCLASS_WEAPON_FIST_WEAPON;
            if (offHand)
            {
                if (offHand->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON)
                {
                    itemSubclass = offHand->GetTemplate()->GetSubClass();

                    uint32 visibleItemId = offHand->GetVisibleEntry(referencePlayer);
                    uint32 itemAppearanceId = offHand->GetVisibleAppearanceModId(referencePlayer);
                    if (ItemModifiedAppearanceEntry const* itemModifiedAppearance = sDB2Manager.GetItemModifiedAppearance(visibleItemId, itemAppearanceId))
                        if (ItemModifiedAppearanceExtraEntry const* itemModifiedAppearaceExtra = sItemModifiedAppearanceExtraStore.LookupEntry(itemModifiedAppearance->ID))
                            if (itemModifiedAppearaceExtra->DisplayWeaponSubclassID > 0)
                                itemSubclass = itemModifiedAppearaceExtra->DisplayWeaponSubclassID;
