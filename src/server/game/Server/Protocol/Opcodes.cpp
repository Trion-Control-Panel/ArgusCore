...
#define DEFINE_HANDLER(opcode, status, processing, handler) \
    ...
    DEFINE_HANDLER(CMSG_ARTIFACT_ADD_RELIC_TALENT,                          STATUS_UNHANDLED, PROCESS_INPLACE,      &WorldSession::Handle_NULL);
    DEFINE_HANDLER(CMSG_ARTIFACT_ATTUNE_PREVIEW_RELIC,                      STATUS_UNHANDLED, PROCESS_INPLACE,      &WorldSession::Handle_NULL);
    ...
    DEFINE_HANDLER(CMSG_AUCTION_LIST_OWNED_ITEMS,                           STATUS_LOGGEDIN,  PROCESS_THREADUNSAFE, &WorldSession::HandleAuctionListOwnerItems);
    ...
    // STATUS_AUTHED (not STATUS_LOGGEDIN) - a purchase can target an offline character
    // (TargetCharacter differs from the currently active one, delivered by mail), the same reason
    // GetProductList needed STATUS_AUTHED for the character-select preview. See ARGUSCORE_FIXES.md.
    ...
    // STATUS_AUTHED (not STATUS_LOGGEDIN) - this is the character-select Character Boost redemption
    // flow, sent before the target character (new or existing) is ever logged into. Confirmed real
    // and reachable in AshamaneCore (HandleBattlePayDistributionAssign, not dead code - the opcode
    // dispatch itself was just never wired there either). See ARGUSCORE_FIXES.md.
    ...
    // STATUS_AUTHED (not STATUS_LOGGEDIN) - matches both reference cores. The Shop's glue-screen
    // (character select) preview queries this with no in-world Player object yet; STATUS_LOGGEDIN
    // requeued/silently dropped those requests forever (masked in-world, where _player always
    // exists by the time the Shop is opened). See ARGUSCORE_FIXES.md.
    ...
    // Root cause of "stuck at Connecting, please wait": this was entirely unwired, and
    // CMSG_BATTLE_PAY_START_PURCHASE had wrongly been assigned this opcode's numeric value (0x36F2) -
    // the real client sends this one for a purchase, not START_PURCHASE (0x36C8, previously missing
    // from the opcode table entirely). See ARGUSCORE_FIXES.md.
    ...
    DEFINE_HANDLER(CMSG_BATTLE_PET_CLEAR_FANFARE,                           STATUS_UNHANDLED, PROCESS_THREADUNSAFE, &WorldSession::HandleBattlePetClearFanfare);
    ...
    DEFINE_HANDLER(CMSG_CALENDAR_COMMUNITY_INVITE,                          STATUS_LOGGEDIN,  PROCESS_THREADUNSAFE, &WorldSession::HandleCalendarGuildFilter);
    ...
    DEFINE_HANDLER(CMSG_GUILD_BANK_SWAP_ITEMS,                              STATUS_UNHANDLED, PROCESS_THREADUNSAFE, &WorldSession::Handle_NULL);
    ...
    DEFINE_HANDLER(CMSG_QUERY_QUEST_REWARDS,                                STATUS_UNHANDLED, PROCESS_INPLACE,      &WorldSession::Handle_NULL);
    ...
    DEFINE_HANDLER(CMSG_QUEST_CONFIRM_ACCEPT,                               STATUS_LOGGEDIN,  PROCESS_INPLACE,      &WorldSession::HandleQuestConfirmAccept);
    ...
    DEFINE_HANDLER(CMSG_REQUEST_LFG_LIST_BLACKLIST,                         STATUS_LOGGEDIN,  PROCESS_INPLACE,      &WorldSession::HandleRequestLfgListBlacklist);
...
#define DEFINE_SERVER_OPCODE_HANDLER(opcode, status, con) \
    ...
    DEFINE_SERVER_OPCODE_HANDLER(SMSG_ARTIFACT_ATTUNE_SOCKETED_RELIC_DATA,     STATUS_NEVER,        CONNECTION_TYPE_REALM);
    DEFINE_SERVER_OPCODE_HANDLER(SMSG_ARTIFACT_KNOWLEDGE,                      STATUS_NEVER,        CONNECTION_TYPE_REALM);
    DEFINE_SERVER_OPCODE_HANDLER(SMSG_ARTIFACT_RESPEC_CONFIRM,                 STATUS_NEVER,        CONNECTION_TYPE_REALM);
    ...
    // These 10 SMSG_LFG_LIST_* (Group Finder) opcodes all have working handlers/Write()
    // implementations (see LFGListHandler.cpp/GroupFinderPackets.cpp) but were left at the
    // DEFINE_SERVER_OPCODE_HANDLER macro's default STATUS_UNHANDLED, which WorldSession::SendPacket
    // treats as "not implemented" and silently drops - same bug class already fixed once for
    // SMSG_ARTIFACT_RESPEC_CONFIRM (see ARGUSCORE_FIXES.md). STATUS_NEVER is the correct value for
    // a server-only opcode, matching the already-working SMSG_LFG_UPDATE_STATUS/
    // SMSG_LFG_PROPOSAL_UPDATE/SMSG_LFG_ROLE_CHECK_UPDATE (regular Dungeon Finder) opcodes above.
