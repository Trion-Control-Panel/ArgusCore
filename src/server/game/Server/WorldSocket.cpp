...
bool WorldSocket::ReadHeaderHandler()
{
    ...
    if (!header->IsValidSize())
    {
        TC_LOG_ERROR("network", "WorldSocket::ReadHeaderHandler(): client {} sent malformed packet (size: {}, cmd: {})",
            GetRemoteIpAddress(), header->Size, header->Command);
        return false;
    }
...
WorldSocket::ReadDataHandlerResult WorldSocket::ReadDataHandler()
{
    ...
    switch (opcode)
    {
        ...
        default:
        {
            ...
            if (!handler)
            {
                TC_LOG_WARN("network.opcode", "No defined handler for opcode {} sent by {}", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet.GetOpcode())), _worldSession->GetPlayerInfo());
                break;
            }
