...
void WorldSession::SendConnectToInstance(WorldPackets::Auth::ConnectToSerial serial)
{
    ...
    else
    {
        ...
        if (v6.is_loopback())
        {
            memcpy(connectTo.Payload.Where.data(), boost::asio::ip::address_v4::loopback().to_bytes().data(), 4);
            connectTo.Payload.Type = WorldPackets::Auth::ConnectTo::IPv4;
        }
        else if (v6.is_v4_mapped())
        {
            memcpy(connectTo.Payload.Where.data(), Trinity::Net::make_address_v4(Trinity::Net::v4_mapped, v6).to_bytes().data(), 4);
            connectTo.Payload.Type = WorldPackets::Auth::ConnectTo::IPv4;
        }
        else
        {
            memcpy(connectTo.Payload.Where.data(), v6.to_bytes().data(), 16);
