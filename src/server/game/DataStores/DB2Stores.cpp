...
using StorageMap = std::map<uint32 /*hash*/, DB2StorageBase*>;
using AreaGroupMemberContainer = std::unordered_map<uint32 /*areaGroupId*/, std::vector<uint32/*areaId*/>>;
using ArtifactPowersContainer = std::unordered_map<uint32, std::vector<ArtifactPowerEntry const*>>;
using ArtifactPowerLinksContainer = std::unordered_map<uint32, std::vector<uint32>>;
using ArtifactPowerRanksContainer = std::unordered_map<std::pair<uint32, uint8>, ArtifactPowerRankEntry const*>;
using CharStartOutfitContainer = std::unordered_map<uint32, CharStartOutfitEntry const*>;
using ChrSpecializationByIndexContainer = ChrSpecializationEntry const*[MAX_CLASSES + 1][MAX_SPECIALIZATIONS];
using ChrSpecialzationByClassContainer = std::unordered_map<uint32, ChrSpecializationEntry const*>;
using CurvePointsContainer = std::unordered_map<uint32 /*curveID*/, std::vector<DBCPosition2D>>;
using EmotesTextSoundContainer = std::map<std::tuple<uint32, uint8, uint8, uint8>, EmotesTextSoundEntry const*>;
using FactionTeamContainer = std::unordered_map<uint32, std::vector<uint32>>;
using HeirloomItemsContainer = std::unordered_map<uint32, HeirloomEntry const*>;
using GlyphBindableSpellsContainer = std::unordered_map<uint32 /*glyphPropertiesId*/, std::vector<uint32>>;
using GlyphRequiredSpecsContainer = std::unordered_map<uint32 /*glyphPropertiesId*/, std::vector<ChrSpecialization>>;
...
using ItemLimitCategoryConditionContainer = std::unordered_map<uint32, std::vector<ItemLimitCategoryConditionEntry const*>>;
...
using ItemBonusTreeContainer = std::unordered_map<uint32, std::set<ItemBonusTreeNodeEntry const*>>;
using ItemSetSpellContainer = std::unordered_map<uint32, std::vector<ItemSetSpellEntry const*>>;
...
using MountCapabilitiesByTypeContainer = std::unordered_map<uint32, DB2Manager::MountTypeXCapabilitySet>;
using MountDisplaysCointainer = std::unordered_map<uint32, DB2Manager::MountXDisplayContainer>;
using NameGenContainer = std::unordered_map<uint32, std::array<std::vector<NameGenEntry const*>, 2>>;
using NameValidationRegexContainer = std::array<std::vector<Trinity::wregex>, TOTAL_LOCALES + 1>;
using PhaseGroupContainer = std::unordered_map<uint32, std::vector<uint32>>;
using PowerTypesContainer = std::array<PowerTypeEntry const*, MAX_POWERS>;
using PvpTalentsByPosition = std::vector<PvpTalentEntry const*>[MAX_CLASSES][MAX_PVP_TALENT_TIERS][MAX_PVP_TALENT_COLUMNS];
using QuestPackageItemContainer = std::unordered_map<uint32, std::pair<std::vector<QuestPackageItemEntry const*>, std::vector<QuestPackageItemEntry const*>>>;
using RulesetItemUpgradeContainer = std::unordered_map<uint32, uint32>;
using SkillRaceClassInfoContainer = std::unordered_multimap<uint32, SkillRaceClassInfoEntry const*>;
using SpecializationSpellsContainer = std::unordered_map<uint32, std::vector<SpecializationSpellsEntry const*>>;
using SpellPowerContainer = std::unordered_map<uint32, std::vector<SpellPowerEntry const*>>;
using SpellPowerDifficultyContainer = std::unordered_map<uint32, std::unordered_map<uint32, std::vector<SpellPowerEntry const*>>>;
using SpellProcsPerMinuteModContainer = std::unordered_map<uint32, std::vector<SpellProcsPerMinuteModEntry const*>>;
using TalentsByPosition = std::vector<TalentEntry const*>[MAX_CLASSES][MAX_TALENT_TIERS][MAX_TALENT_COLUMNS];
using ToyItemIdsContainer = std::unordered_set<uint32>;
using WMOAreaTableKey = std::tuple<uint16, uint8, int32>;
using WMOAreaTableLookupContainer = std::map<WMOAreaTableKey, WMOAreaTableEntry const*>;
using WorldMapAreaByAreaIDContainer = std::unordered_map<uint32, WorldMapAreaEntry const*>;
using HotfixBlobKey = std::pair<uint32 /*tableHash*/, int32 /*recordId*/>;
using HotfixBlobMap = std::map<HotfixBlobKey, std::vector<uint8>>;
