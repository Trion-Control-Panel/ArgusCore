...
namespace
{
    EnumFlag<ToyFlags> GetToyFlags(bool isFavourite)
    {
...
void CollectionMgr::LoadAccountToys(PreparedQueryResult result)
{
    ...
    do
    {
        ...
        _toys.emplace(itemId, GetToyFlags(fields[1].GetBool()));
    } while (result->NextRow());
}
...
{
    return _toys.insert(ToyBoxContainer::value_type(itemId, GetToyFlags(isFavourite))).second;
}
...
void CollectionMgr::UpgradeHeirloom(uint32 itemId, int32 castItem)
{
    ...
    uint16 offset = uint16(std::ranges::distance(heirlooms.begin(), std::ranges::find(heirlooms, itemId)));
...
void CollectionMgr::LoadAccountItemAppearances(PreparedQueryResult knownAppearances, PreparedQueryResult favoriteAppearances)
{
    if (knownAppearances)
    {
        ...
        // init_from_block_range became private in Boost 1.87+. The public equivalent
        // is the range constructor — build a new bitset from the raw blocks and assign it.
