...
void Unit::HandleEmoteCommand(Emote emoteId, Player* target /*=nullptr*/)
{
...
void Unit::SetPowerType(Powers power, bool sendUpdate/* = true*/, bool onInit /*= false*/)
{
    ...
    SetUInt32Value(UNIT_FIELD_DISPLAY_POWER, power);
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    if (Player const* thisPlayer = ToPlayer())
    {
        float overrideSP = thisPlayer->GetFloatValue(PLAYER_FIELD_OVERRIDE_SPELL_POWER_BY_AP_PCT);
        if (overrideSP > 0.0f)
    ...
    *data << uint8(blockCount);
    ...
    for (uint16 index = 0; index < valCount; ++index)
    {
        ...
        if (shouldUpdate)
        {
            ...
            else if (index == UNIT_FIELD_FLAGS_3)
            {
                uint32 appendValue = m_uint32Values[UNIT_FIELD_FLAGS_3];
                if (appendValue & UNIT_FLAG3_ALREADY_SKINNED && creature && creature->IsSkinnedBy(target))
                    appendValue &= ~UNIT_FLAG3_ALREADY_SKINNED;
