...
void Item::SaveToDB(CharacterDatabaseTransaction trans)
{
    ...
    switch (uState)
    {
        ...
        case ITEM_CHANGED:
        {
            ...
            stmt->setUInt32(++index, GetExpiration());
            ...
            for (uint32 bonusListID : GetBonusListIDs())
...
bool Item::GemsFitSockets() const
{
    for (uint32 gemSlot = 0; gemSlot < MAX_ITEM_PROTO_SOCKETS; ++gemSlot)
    {
        ...
        if (gemSlot >= GetGems().size()) // no gems on this socket
            return false;

        uint8 GemColor = 0;

        uint32 gemid = GetGem(gemSlot)->ItemId;
        if (gemid)
        {
...
uint32 Item::GetItemLevel(Player const* owner) const
{
    ...
    uint32 maxItemLevel = pvpBonus && itemTemplate->HasFlag(ITEM_FLAG3_IGNORE_ITEM_LEVEL_CAP_IN_PVP) ? 0 : owner->GetUInt32Value(UNIT_FIELD_MAXITEMLEVEL);
...
uint32 Item::GetTotalUnlockedArtifactPowers() const
{
    ...
    uint64 artifactXp = GetUInt64Value(ITEM_FIELD_ARTIFACT_XP);
