...
std::vector<int32> GetBonusListsForItem(uint32 itemId, ItemBonusGenerationParams const& params)
{
    ...
    for (auto itemTreeItr = itemIdRange.first; itemTreeItr != itemIdRange.second; ++itemTreeItr)
    {
        ...
        VisitItemBonusTree(itemTreeItr->second, false, [params, &matchingNodes](ItemBonusTreeNodeEntry const* bonusTreeNode)
        {
            if (ItemContext(bonusTreeNode->ItemContext) == ItemContext::NONE || params.Context == ItemContext(bonusTreeNode->ItemContext))
                ++matchingNodes;
        });
        ...
        VisitItemBonusTree(itemTreeItr->second, true, [params, &bonusListIDs, &itemLevelSelectorId](ItemBonusTreeNodeEntry const* bonusTreeNode)
        {
            ...
            if (ItemContext(bonusTreeNode->ItemContext) != ItemContext::NONE && params.Context != requiredContext)
                return;
