    ...
    *data << uint32(PauseTimes ? PauseTimes->size() : 0);
    ...
    if (flags.AreaTrigger)
    {
        ...
        *data << areaTrigger->GetRollPitchYaw().PositionXYZStream();
        ...
        bool hasVisualAnimIsDecay   = createProperties && createProperties->Flags.HasFlag(AreaTriggerCreatePropertiesFlag::VisualAnimIsDecay);
        ...
        data->WriteBit(hasVisualAnimIsDecay);
...
{
    TC_LOG_ERROR("misc", "Attempt {} non-existed value field: {} (count: {}) for object typeid: {} type mask: {}", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
void WorldObject::PlayDirectSound(uint32 soundId, Player const* target /*= nullptr*/) const
{
    if (target)
...
void WorldObject::PlayObjectSound(int32 soundKitId, ObjectGuid targetObjectGUID, Player const* target /*= nullptr*/) const
{
