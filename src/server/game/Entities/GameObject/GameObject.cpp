...
public:
    ...
    void Update(uint32 diff) override
    {
        ...
        else
        {
            ...
            if (now < _owner.GetUInt32Value(GAMEOBJECT_LEVEL))
            {
        ...
        if (shouldUpdate)
        {
            ...
            if (index == OBJECT_DYNAMIC_FLAGS)
            {
                ...
                if (!target->IsGameMaster())
                {
                    ...
                    if (GetGoType() != GAMEOBJECT_TYPE_GATHERING_NODE && GetGoType() != GAMEOBJECT_TYPE_FLAGSTAND && GetGoType() != GAMEOBJECT_TYPE_SPELLCASTER)
                    ...
                    if (SpawnMetadata const* data = sObjectMgr->GetSpawnMetadata(SPAWN_TYPE_GAMEOBJECT, GetSpawnId()))
                        if (data->spawnTrackingData && !data->spawnTrackingQuestObjectives.empty())
                            if (target->GetSpawnTrackingStateByObjectives(data->spawnTrackingData->SpawnTrackingId, data->spawnTrackingQuestObjectives) != SpawnTrackingState::Active)
                                dynFlags &= ~GO_DYNFLAG_LO_ACTIVATE;
                }
            ...
            {
                uint32 bytes1 = m_uint32Values[GAMEOBJECT_BYTES_1];
...
void GameObject::SetPathProgressForClient(float progress)
{
    ...
    {
        bool marked = _changesMask[OBJECT_DYNAMIC_FLAGS] > 0;
        ...
        if (!marked)
            _changesMask[OBJECT_DYNAMIC_FLAGS] = 0;
    });
}
