...
void WorldSession::HandleLfgLeaveOpcode(WorldPackets::LFG::DFLeave& dfLeave)
{
    ...
    // Check cheating - grouped players may only leave via their group leader's ticket;
    // solo players may only leave their own ticket, not an arbitrary player's.
...
void WorldSession::SendLfgJoinResult(lfg::LfgJoinResultData const& joinData)
{
    ...
    for (lfg::LfgLockPartyMap::const_iterator it = joinData.lockmap.begin(); it != joinData.lockmap.end(); ++it)
    {
        ...
        for (lfg::LfgLockMap::const_iterator itr = it->second.begin(); itr != it->second.end(); ++itr)
        {
            ...
            blackList.Slot.emplace_back(itr->first, itr->second.lockStatus, itr->second.requiredItemLevel, itr->second.currentItemLevel);
        }
    }
