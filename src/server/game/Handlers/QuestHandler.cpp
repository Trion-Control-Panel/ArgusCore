...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPackets::Quest::QuestGiverChooseReward& packet)
{
    TC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc = {}, quest = {}, reward = {}",
        packet.QuestGiverGUID.ToString(), packet.QuestID, packet.ItemChoiceID);
            ...
            case LootItemType::Item:
            {
                ...
                if (!rewardProto)
                {
                    TC_LOG_ERROR("entities.player.cheat", "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player {} {} tried to get invalid reward item (Item Entry: {}) for quest {} (possible packet-hacking detected)",
                        _player->GetName(), _player->GetGUID().ToString(), packet.ItemChoiceID, packet.QuestID);
                    return;
                }
    ...
    {
        if (_player->CanRewardQuest(quest, packet.ChoiceLootItemType, packet.ItemChoiceID, true)) // Then check if player can receive the reward item (if inventory is not full, if player doesn't have too many unique items, and so on). If not, the client will close the gossip window
        {
