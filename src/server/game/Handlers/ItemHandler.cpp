...
void WorldSession::HandleSellItemOpcode(WorldPackets::Item::SellItem& packet)
{
    ...
    if (pItem)
    {
        ...
        if (uint32 sellPrice = pItem->GetSellPrice(_player); sellPrice > 0)
        {
            ...
            if (money > std::numeric_limits<uint32>::max()) // ensure sell price * amount doesn't overflow buyback price
            {
                _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, creature, packet.ItemGUID);
                return;
            }
...
void WorldSession::HandleUpgradeItem(WorldPackets::Item::UpgradeItem& upgradeItem)
{
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItems - {} not found or player can't interact with it.", upgradeItem.ItemMaster.ToString());
    ...
    if (!item)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItems: Item {} not found!", upgradeItem.ItemGUID.ToString());
        itemUpgradeResult.Success = false;
        SendPacket(itemUpgradeResult.Write());
        return;
    }
    ...
    if (!itemUpgradeEntry)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItems - ItemUpgradeEntry ({}) not found.", upgradeItem.UpgradeID);
        itemUpgradeResult.Success = false;
        SendPacket(itemUpgradeResult.Write());
        return;
    }
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItems - Player has not enougth currency (ID: {}, Cost: {}) not found.", itemUpgradeEntry->CurrencyType, itemUpgradeEntry->CurrencyAmount);
        itemUpgradeResult.Success = false;
        SendPacket(itemUpgradeResult.Write());
        return;
    }
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItems - ItemUpgradeEntry ({}) is not related to this ItemUpgradePath ({}).", itemUpgradeEntry->ID, currentUpgradeId);
        itemUpgradeResult.Success = false;
        SendPacket(itemUpgradeResult.Write());
        return;
    }
