        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_FLAG_SET:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    {
                        TC_LOG_ERROR("scripts", "{} wrong field {} (max count: {}) in object (TypeId: {}, Entry: {}, {}) specified, skipping.",
                            step.script->GetDebugInfo(), step.script->FlagToggle.FieldID,
                            cSource->GetValuesCount(), cSource->GetTypeId(), cSource->GetEntry(), cSource->GetGUID().ToString());
                    }
                    else
