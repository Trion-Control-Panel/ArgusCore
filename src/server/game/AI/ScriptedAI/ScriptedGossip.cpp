...
{
    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, optionNpc, std::move(text), 0, 0, false, 0, "", sender, action);
}
...
{
    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, optionNpc, std::move(text), 0, 0, coded, popupMoney, std::move(popupText), sender, action);
}
