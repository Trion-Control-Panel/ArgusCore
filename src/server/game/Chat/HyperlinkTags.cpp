...
bool Trinity::Hyperlinks::LinkTags::item::StoreTo(ItemLinkData& val, std::string_view text)
{
    ...
    if (!(val.Item && t.TryConsumeTo(val.EnchantId) && t.TryConsumeTo(val.GemItemId[0]) && t.TryConsumeTo(val.GemItemId[1]) &&
        t.TryConsumeTo(val.GemItemId[2]) && t.TryConsumeTo(dummy) && !dummy && t.TryConsumeTo(val.RandomPropertyId) && t.TryConsumeTo(val.RandomPropertySeed) &&
        t.TryConsumeTo(val.RenderLevel) && t.TryConsumeTo(val.RenderSpecialization) && t.TryConsumeTo(modifiersMask) &&
    ...
    for (uint32 i = 0; i < MAX_ITEM_MODIFIERS; ++i)
    {
        ...
        {
            int32 id;
            if (!t.TryConsumeTo(id))
                return false;

            ItemLinkData::Modifier modifier;
            modifier.Type = i;
            modifier.Value = id;
            val.Modifiers.emplace_back(modifier);
        }
    }
