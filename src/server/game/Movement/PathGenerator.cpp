...
bool PathGenerator::TryInsertBypassAt(uint32 segmentIndex)
{
    // Build a bypass waypoint perpendicular to the blocked segment so the unit
    // can walk around the obstacle instead of stopping dead in front of it.
    // We try both sides at increasing lateral offsets until we find a position
    // with clear LOS on both legs (current->bypass and bypass->next).
    G3D::Vector3 from = _pathPoints[segmentIndex];
    G3D::Vector3 to   = _pathPoints[segmentIndex + 1];

    // Horizontal direction along the blocked segment.
    G3D::Vector3 dir = to - from;
    dir.z = 0.0f;
    float const segLen = dir.length();
    if (segLen < 0.001f)
        return false;

    dir = dir / segLen;

    // Perpendicular to the segment in the XY plane (left-hand and right-hand).
    G3D::Vector3 const perp(-dir.y, dir.x, 0.0f);

    float const halfHeight = _source->GetCollisionHeight() * 0.5f;
    G3D::Vector3 const mid  = (from + to) * 0.5f;

    float const offsets[] = {2.0f, 4.0f, 6.0f, 8.0f};
    float const sides[]   = {1.0f, -1.0f};

    for (float offset : offsets)
    {
        for (float side : sides)
        {
            G3D::Vector3 bypass = mid + perp * (offset * side);
            // Snap the bypass point to walkable ground height at that XY position.
            _source->UpdateAllowedPositionZ(bypass.x, bypass.y, bypass.z);

            // Skip if the bypass Z ended up far from the segment midpoint Z.
            // That means the ground there is on a completely different level
            // (underground, rooftop, etc.) and would look wrong.
            if (std::abs(bypass.z - mid.z) > 3.0f)
                continue;

            // Both legs of the detour must be unobstructed.
            if (!_source->GetMap()->isInLineOfSight(_source->GetPhaseShift(),
                    from.x,   from.y,   from.z   + halfHeight,
                    bypass.x, bypass.y, bypass.z + halfHeight,
            ...
            // Valid bypass found. Insert it between segmentIndex and segmentIndex+1.
            _pathPoints.insert(_pathPoints.begin() + segmentIndex + 1, bypass);
            SetActualEndPosition(_pathPoints.back());

            TC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::TryInsertBypassAt: bypass inserted at segment {} offset {:.1f} side {:.0f}", segmentIndex, offset, side);
            return true;
        }
    }
...
void PathGenerator::ValidatePathAgainstCollision()
{
    ...
    // Cap how many bypasses we allow in a single path build. Each bypass adds
    // two extra LOS checks and one more path point, so we do not want unbounded
    // retries if the area is full of thin objects.
    uint32 const maxBypasses = 3;
    uint32 bypassCount = 0;
    ...
    while (i + 1 < _pathPoints.size())
    {
        ...
        TC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::ValidatePathAgainstCollision: segment {} blocked by vmap", i);

        // Try to route around the obstacle before giving up.
        if (bypassCount < maxBypasses && TryInsertBypassAt(i))
        {
            ++bypassCount;
            // Do not advance i. Re-check the new from->bypass segment on the
            // next iteration to make sure it is actually clear.
            continue;
        }

        // No bypass found. Stop the unit before the obstacle rather than
