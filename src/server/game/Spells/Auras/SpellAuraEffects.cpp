...
NonDefaultConstructible<pAuraEffectHandler> AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleUnused,                                    //272 unused (7.3.5)
    ...
    &AuraEffect::HandleUnused,                                    //285 unused (7.3.5)
    &AuraEffect::HandleUnused,                                    //286 unused (7.3.5) old SPELL_AURA_ABILITY_PERIODIC_CRIT
    ...
    &AuraEffect::HandleUnused,                                    //334 unused
    ...
    &AuraEffect::HandleUnused,                                    //339 unused
    ...
    &AuraEffect::HandleModPossessPet,                             //378 SPELL_AURA_MOD_POSSESS_PET TheLegionPreservationProject: seems unused
...
void AuraEffect::PeriodicTick(AuraApplication* aurApp, Unit* caster) const
{
    ...
    if (caster && !caster->IsPlayer() && m_spellInfo->IsChanneled() && m_spellInfo->HasAttribute(SPELL_ATTR1_TRACK_TARGET_IN_CHANNEL) && caster->GetChannelObjects().size())
    {
        ObjectGuid const channelGuid = *caster->GetChannelObjects().begin();
...
void AuraEffect::HandleAuraTrackResources(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
        target->AddTrackResourceFlag(flag);
    else
        target->RemoveTrackResourceFlag(flag);
}
