...
void Spell::DoCreateItem(uint32 itemId, ItemContext context /*= ItemContext::NONE*/, std::vector<int32> const* bonusListIDs /*= nullptr*/)
{
    ...
    if (num_to_add)
    {
        ...
        if (Item* pItem = player->StoreNewItem(dest, newitemid, true, GenerateItemRandomPropertyId(newitemid), GuidSet(), context, bonusListIDs))
        {
...
void Spell::EffectGiveExperience()
{
    ...
    // TODO: misc value A on 178188?

    uint32 xp = Quest::XPValue(playerTarget, playerTarget->GetLevel(), effectInfo->MiscValueB);
