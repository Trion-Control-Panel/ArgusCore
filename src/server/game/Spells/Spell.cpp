    ...
    while (chainTargets)
    {
        ...
        if (isChainHeal)
        {
            ...
            for (std::list<WorldObject*>::iterator itr = tempTargets.begin(); itr != tempTargets.end(); ++itr)
            {
                if (Unit* unit = (*itr)->ToUnit())
                {
                    ...
                    if ((deficit > maxHPDeficit || foundItr == tempTargets.end()) && chainSource->IsWithinDist(unit, jumpRadius) && IsWithinLOS(chainSource, unit, false, VMAP::ModelIgnoreFlags::Nothing))
                    {
                        foundItr = itr;
        ...
        else
        {
            for (std::list<WorldObject*>::iterator itr = tempTargets.begin(); itr != tempTargets.end(); ++itr)
            {
                ...
                if (!IsWithinLOS(chainSource, *itr, false, VMAP::ModelIgnoreFlags::Nothing))
                    continue;

                if (spellEffectInfo.EffectAttributes.HasFlag(SpellEffectAttributes::EnforceLineOfSightToChainTargets) && !IsWithinLOS(m_caster, *itr, false, VMAP::ModelIgnoreFlags::Nothing))
                    continue;
...
SpellCastResult Spell::prepare(SpellCastTargets const& targets, AuraEffect const* triggeredByAura)
{
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_TIME))
        m_casttime = m_spellInfo->CalcCastTime(cast_time_level, this);
...
void Spell::SendCastResult(SpellCastResult result, int32* param1 /*= nullptr*/, int32* param2 /*= nullptr*/) const
{
    ...
    castFailed.SpellXSpellVisualID = m_SpellVisual.SpellXSpellVisualID;
...
void Spell::SendCastResult(Player* caster, SpellInfo const* spellInfo, SpellCastVisual spellVisual, ObjectGuid cast_count, SpellCastResult result, SpellCustomErrors customError /*= SPELL_CUSTOM_ERROR_NONE*/, int32* param1 /*= nullptr*/, int32* param2 /*= nullptr*/)
{
    ...
    packet.SpellXSpellVisualID = spellVisual.SpellXSpellVisualID;
...
void Spell::SendSpellStart()
{
    ...
    castData.SpellXSpellVisualID = m_SpellVisual.SpellXSpellVisualID;
...
void Spell::SendInterrupted(uint8 result)
{
    ...
    failurePacket.SpellXSpellVisualID = m_SpellVisual.SpellXSpellVisualID;
    ...
    failedPacket.SpellXSpellVisualID = m_SpellVisual.SpellXSpellVisualID;
...
void Spell::SendChannelUpdate(uint32 time, Optional<SpellCastResult> /*result*/)
{
    ...
    if (time == 0)
    {
        ...
        unitCaster->SetChannelSpellXSpellVisualId({});
    }
...
void Spell::SendResurrectRequest(Player* target)
{
    ...
    if (Player const* playerCaster = m_caster->ToPlayer())
        resurrectRequest.ResurrectOffererVirtualRealmAddress = playerCaster->GetVirtualPlayerRealm();
    else
...
SpellCastResult Spell::CheckCast(bool strict, int32* param1 /*= nullptr*/, int32* param2 /*= nullptr*/)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetTypeId() != TYPEID_GAMEOBJECT)
            {
                ...
                if (!IsWithinLOS(losTarget, target, true, VMAP::ModelIgnoreFlags::Nothing))
    ...
    if (m_targets.HasDst())
        if (!IsWithinLOS(m_caster, *m_targets.GetDstPos(), VMAP::ModelIgnoreFlags::Nothing))
            return SPELL_FAILED_LINE_OF_SIGHT;
    ...
    switch (spellEffectInfo.Effect)
    {
        ...
        default:
        {
            if (!losPosition || m_spellInfo->HasAttribute(SPELL_ATTR5_ALWAYS_AOE_LINE_OF_SIGHT) || spellEffectInfo.EffectAttributes.HasFlag(SpellEffectAttributes::AlwaysAoeLineOfSight))
            {
                ...
                if (target != m_caster && !IsWithinLOS(caster, target, true, VMAP::ModelIgnoreFlags::Nothing))
                    return false;
            }

            if (losPosition)
                if (!IsWithinLOS(target, *losPosition, VMAP::ModelIgnoreFlags::Nothing))
                    return false;
        }
    }
