...
void AuctionBotSeller::SetPricesOfItem(ItemTemplate const* itemProto, SellerConfiguration& config, uint32& buyout, uint32& bid, uint32 stackCount)
{
    ...
    buyout = static_cast<uint32>(frand(basePriceFloat - range, basePriceFloat + range) + 0.5f);
    if (buyout == 0)
        buyout = SILVER;
    ...
    bid = static_cast<uint32>(bidPercentage * buyout);
    if (bid == 0)
        bid = SILVER;
}
