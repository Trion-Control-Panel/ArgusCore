...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    //                                        40           41          42
    //                                       "flags_extra, ScriptName, StringId FROM creature_template WHERE entry = ? OR 1 = ? "
...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.RegenHealth            = fields[38].GetBool();
    creatureTemplate.CreatureImmunitiesId   = fields[39].GetInt32();
    creatureTemplate.flags_extra            = fields[40].GetUInt32();
    creatureTemplate.ScriptID               = GetScriptId(fields[41].GetString());
    creatureTemplate.StringId               = fields[42].GetString();
}
...
void ObjectMgr::LoadCreatureTemplateModels()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Creature (Entry: {}) lists CreatureDisplayID id ({}) at Idx ({}), max supported Idx is 3. Skipped!.", creatureId, creatureDisplayId, idx);
            continue;
        }
...
void ObjectMgr::LoadCreatureTemplateDifficulty()
{
    ...
    //                                                   0      1             2         3         4                5                6                     7                     8
    QueryResult result = WorldDatabase.Query("SELECT Entry, DifficultyID, MinLevel, MaxLevel, LevelScalingMin, LevelScalingMax, LevelScalingDeltaMin, LevelScalingDeltaMax, HealthScalingExpansion, "
    //   9               10            11             12              13                    14         15
    ...
    do
    {
        ...
        creatureDifficulty.MinLevel               = fields[2].GetUInt32();
        creatureDifficulty.MaxLevel               = fields[3].GetUInt32();
        creatureDifficulty.LevelScalingMin        = fields[4].GetUInt32();
        creatureDifficulty.LevelScalingMax        = fields[5].GetUInt32();
        creatureDifficulty.DeltaLevelMin          = fields[6].GetInt16();
        creatureDifficulty.DeltaLevelMax          = fields[7].GetInt16();
        creatureDifficulty.HealthScalingExpansion = fields[8].GetInt32();
        creatureDifficulty.HealthModifier         = fields[9].GetFloat();
        creatureDifficulty.ManaModifier           = fields[10].GetFloat();
        creatureDifficulty.ArmorModifier          = fields[11].GetFloat();
        creatureDifficulty.DamageModifier         = fields[12].GetFloat();
        creatureDifficulty.CreatureDifficultyID   = fields[13].GetInt32();
        creatureDifficulty.TypeFlags              = fields[14].GetUInt32();
        creatureDifficulty.TypeFlags2             = fields[15].GetUInt32();
        creatureDifficulty.LootID                 = fields[16].GetUInt32();
        creatureDifficulty.PickPocketLootID       = fields[17].GetUInt32();
        creatureDifficulty.SkinLootID             = fields[18].GetUInt32();
        creatureDifficulty.GoldMin                = fields[19].GetUInt32();
        creatureDifficulty.GoldMax                = fields[20].GetUInt32();
        creatureDifficulty.StaticFlags            = CreatureStaticFlagsHolder(CreatureStaticFlags(fields[21].GetUInt32()), CreatureStaticFlags2(fields[22].GetUInt32()),
            CreatureStaticFlags3(fields[23].GetUInt32()), CreatureStaticFlags4(fields[24].GetUInt32()), CreatureStaticFlags5(fields[25].GetUInt32()),
            CreatureStaticFlags6(fields[26].GetUInt32()), CreatureStaticFlags7(fields[27].GetUInt32()),  CreatureStaticFlags8(fields[28].GetUInt32()));
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    for (std::set<uint32>::const_iterator itr = notFoundOutfit.begin(); itr != notFoundOutfit.end(); ++itr)
        TC_LOG_ERROR("sql.sql", "Item (Entry: {}) does not exist but is referenced in `CharStartOutfit.dbc`", *itr);
...
void ObjectMgr::LoadItemTemplateAddon()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT Id, FlagsCu, FoodType, MinMoneyLoot, MaxMoneyLoot, SpellPPMChance, QuestLogItemId FROM item_template_addon");
    if (result)
    {
        do
        {
            ...
            itemTemplate->QuestLogItemId = fields[6].GetInt32();
            ++count;
        } while (result->NextRow());
    }
    ...
    // Load playercreate
    {
        ...
        else
        {
            ...
            do
            {
                ...
                info->introMovieId = fields[7].GetUInt32OrNull();
                ...
                info->introSceneId = fields[8].GetUInt32OrNull();
...
void ObjectMgr::LoadQuests()
{
    ...
    for (auto& questPair : _questTemplates)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Quest {} should be disabled because `MinLevel` = {}", qinfo->GetQuestId(), int32(qinfo->_minLevel));
...
void ObjectMgr::LoadInstanceFinalEncounter()
{
    uint32 oldMSTime = getMSTime();

    //                                               0                   1
    QueryResult result = WorldDatabase.Query("SELECT JournalEncounterID, FinalDungeonEncounterID FROM instance_final_encounter");
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 instance final encounters, table is empty!");
        return;
    }
    ...
    {
        Field* fields = result->Fetch();
        uint32 journalEncounterId = fields[0].GetUInt32();
        uint32 finalDungeonEncounterId = fields[1].GetUInt32();
        JournalEncounterEntry const* journalEncounter = sJournalEncounterStore.LookupEntry(journalEncounterId);
        if (!journalEncounter)
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_final_encounter` has an invalid journal encounter id {}, skipped!", journalEncounterId);
            continue;
        }

        DungeonEncounterEntry const* dungeonEncounter = sDungeonEncounterStore.LookupEntry(finalDungeonEncounterId);
        if (!dungeonEncounter)
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_final_encounter` has an invalid final dungeon encounter id {}, skipped!", finalDungeonEncounterId);
            continue;
        }

        JournalInstanceEntry const* journalInstance = sJournalInstanceStore.LookupEntry(journalEncounter->JournalInstanceID);
        if (!journalEncounter)
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_final_encounter` failed to locate journal instance for journal encounter id {}, skipped!", journalEncounterId);
            continue;
        }

        if (dungeonEncounter->MapID != journalInstance->MapID)
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_final_encounter` journal encounter id {} and final dungeon encounter id {} refer to different maps, skipped!", journalEncounterId, finalDungeonEncounterId);
            continue;
        }

        _finalDungeonEncounterStore.emplace(journalEncounterId, finalDungeonEncounterId);
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    //                                               0      1           2            3                     4
    QueryResult result = WorldDatabase.Query("SELECT entry, creditType, creditEntry, lastEncounterDungeon, comment FROM instance_encounters");
    if (!result)
    {
    ...
    do
    {
        ...
        switch (creditType)
        {
            case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                // Unlike older cores, ArgusCore's CreatureTemplate is not duplicated per difficulty
                // (that data lives in creature_template_difficulty instead), so flags_extra only
                // needs to be set once per entry - no DifficultyEntry[] loop needed here.
...
AreaTriggerStruct const* ObjectMgr::GetGoBackTrigger(uint32 Map) const
{
    ...
    for (AreaTriggerContainer::const_iterator itr = _areaTriggerStore.begin(); itr != _areaTriggerStore.end(); ++itr)
    {
        if (itr->second.target_mapId == entrance_map)
        {
            ...
            if (atEntry && uint32(atEntry->ContinentID) == Map)
                return &itr->second;
        }
    }
    return nullptr;
...
void ObjectMgr::LoadGameObjectTemplate()
{
    ...
    //                                        37      38       39     40      41      42             43      44          45
                                             "Data29, Data30, Data31, Data32, Data33, RequiredLevel, AIName, ScriptName, StringId "
    ...
    do
    {
        ...
        got.StringId = fields[45].GetString();
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `points_of_interest` (ID: {}) have invalid coordinates (PositionX: {} PositionY: {}), ignored.",
                id, pointOfInterest.Pos.GetPositionX(), pointOfInterest.Pos.GetPositionY());
            continue;
        }
    ...
    do
    {
        ...
        if (std::map<int32, std::vector<QuestPOIBlobPoint>>* blobs = Trinity::Containers::MapGetValuePtr(allPoints, questID))
        {
            if (std::vector<QuestPOIBlobPoint>* points = Trinity::Containers::MapGetValuePtr(*blobs, idx1))
            {
                ...
                poiData.Blobs.emplace_back(blobIndex, objectiveIndex, questObjectiveID, questObjectID, mapID, worldMapAreaID, floor, priority, flags,
                    worldEffectID, playerConditionID, spawnTrackingID, std::move(*points), alwaysAllowMergingBlobs);
                continue;
            }
        }
    ...
    if (QueryResult trainerLocalesResult = WorldDatabase.Query("SELECT Id, locale, Greeting_lang FROM trainer_locale"))
    {
        do
        {
            ...
            if (Trainer::Trainer* trainer = Trinity::Containers::MapGetValuePtr(_trainers, trainerId))
                trainer->AddGreetingLocale(locale, fields[2].GetStringView());
            else
...
void ObjectMgr::LoadGossipMenuItemAddon()
{
    ...
    do
    {
        ...
        if (Optional<int32> garrTalentTreeId = fields[2].GetInt32OrNull())
        {
            addon.GarrTalentTreeID = *garrTalentTreeId;

            if (!sGarrTalentTreeStore.LookupEntry(*addon.GarrTalentTreeID))
            {
                TC_LOG_ERROR("sql.sql", "Table gossip_menu_option_addon: MenuID {} OptionID {} is using non-existing GarrTalentTree {}",
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} gossip_menu_option_addon IDs in {} ms", uint32(_gossipMenuItemAddonStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadTerrainWorldMaps()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "WorldMapArea {} defined in `terrain_worldmap` does not exist, skipped.", worldMapArea);
            continue;
        }
...
uint32 ObjectMgr::GetFinalDungeonEncounterID(uint32 journalEncounterId) const
{
    auto itr = _finalDungeonEncounterStore.find(journalEncounterId);
...
void ObjectMgr::LoadRaceAndClassExpansionRequirements()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!classEntry)
            {
                TC_LOG_ERROR("sql.sql", "Class {} defined in `class_expansion_requirement` does not exists, skipped.", classID);
                continue;
            }

            if (expansion >= MAX_EXPANSIONS)
            {
                TC_LOG_ERROR("sql.sql", "Class {} defined in `class_expansion_requirement` has incorrect expansion {}, skipped.", classID, expansion);
                continue;
            }
...
void ObjectMgr::LoadSceneTemplates()
{
    ...
    do
    {
        ...
        sceneTemplate.ScriptId          = GetScriptId(fields[3].GetCString());
...
void ObjectMgr::LoadPlayerChoices()
{
    ...
    QueryResult choices = WorldDatabase.Query("SELECT ChoiceId, UiTextureKitId, Question, InfiniteRange, HideWarboardHeader, MaxResponses, ScriptName FROM playerchoice");
    if (!choices)
    {
...

    {
        do
        {
            ...
            choice.Question = fields.Question().GetString();
    ...
    if (QueryResult responses = WorldDatabase.Query("SELECT ChoiceId, ResponseId, ChoiceArtFileId, Header, Answer, Description, Confirmation FROM playerchoice_response ORDER BY `Index` ASC"))
    {
        do
        {
            ...
            response.Header             = fields.Header().GetString();
            response.Answer             = fields.Answer().GetString();
            response.Description        = fields.Description().GetString();
            response.Confirmation       = fields.Confirmation().GetString();
            ++responseCount;
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} player choices, {} responses, {} rewards, {} item rewards, {} currency rewards and {} faction rewards in {} ms.",
...
void ObjectMgr::LoadPlayerChoicesLocale()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT ChoiceID, ResponseID, locale, Header, Answer, Description, Confirmation FROM playerchoice_response_locale"))
    {
        ...
        do
        {
            ...
            AddLocaleString(fields[3].GetStringView(), locale, data.Header);
            AddLocaleString(fields[4].GetStringView(), locale, data.Answer);
            AddLocaleString(fields[5].GetStringView(), locale, data.Description);
            AddLocaleString(fields[6].GetStringView(), locale, data.Confirmation);
            ++count;
        } while (result->NextRow());
