...
#define LOAD_CHECK(fn) do { fn; \
    ...
    std::string certificateChainFile = sConfigMgr->GetStringDefault("CertificatesFile", "./certs/bnetserver.cert.pem");
    ...
    if (sk_X509_num(certs) > 0)
    {
        ...
        {
            // OpenSSL 3.x made these return const pointers for correctness.
            const X509_NAME* nm = X509_get_subject_name(cert);
            int32 lastpos = -1;
            while (true)
            {
                ...
                const ASN1_STRING* text = X509_NAME_ENTRY_get_data(e);
                if (!text)
                    continue;
    ...
    if (!key)
    {
        std::string privateKeyFile = sConfigMgr->GetStringDefault("PrivateKeyFile", "./certs/bnetserver.key.pem");
