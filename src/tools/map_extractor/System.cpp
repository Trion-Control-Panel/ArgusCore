        ...
        if (wdt.loadFile(CascStorage, storagePath, false))
        {
            ...
            for (uint32 y = 0; y < WDT_MAP_SIZE; ++y)
            {
                for (uint32 x = 0; x < WDT_MAP_SIZE; ++x)
                {
                    ...
                    std::string adtStoragePath = Trinity::StringFormat("World\\Maps\\{}\\{}_{}_{}.adt", map_ids[z].Directory, map_ids[z].Directory, x, y);
...
void ExtractGameTables()
{
    ...
    static constexpr DB2FileInfo GameTables[] =
    {
        ...
        { .FileDataId = 1391649, .Name = "NpcDamageByClassExp5.txt" },
...
uint32 GetInstalledLocalesMask()
{
    ...
    {
        boost::filesystem::path const storage_dir(boost::filesystem::canonical(input_path) / "Data");
        std::unique_ptr<CASC::Storage> storage(CASC::Storage::Open(storage_dir, CASC_LOCALE_ALL));
        if (!storage)
            return false;
