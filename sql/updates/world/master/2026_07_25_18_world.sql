...
-- 205484, confirmed via DestinyCore's own committed spell_script_names data - this is
-- DIFFERENT from SPELL_WARRIOR_INSPIRING_PRESENCE (222944), which is the inner heal
-- spell the script casts when it procs, not the id it's registered under.
--
-- Note on spell_warr_precise_strikes: its spell id (248579) comes from DestinyCore's
-- own source comment ("// 248579 - Precise Strikes / 7.3.5"), not an independently
-- confirmed spell_script_names row - DestinyCore's own DB dump has no binding for
-- this script name at all, so this one carries slightly lower confidence than the
-- other six.
