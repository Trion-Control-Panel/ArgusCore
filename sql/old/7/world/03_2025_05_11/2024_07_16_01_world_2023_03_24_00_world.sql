...
# DELETE FROM `gossip_menu_option` WHERE `MenuID` IN (6092,6094,8308,8306) AND `OptionID`=0;
# INSERT INTO `gossip_menu_option`(`GossipOptionID`, `MenuID`, `OptionID`, `OptionNpc`, `OptionText`, `OptionBroadcastTextID`, `ActionMenuID`, `ActionPoiID`, `BoxCoded`, `BoxMoney`, `BoxText`, `BoxBroadcastTextID`, `VerifiedBuild`) VALUES
# (-1559552, 6092, 0, 0, 'This Dimensional Imploder sounds dangerous!  How can I make one?', 9994, 0, 0, 0, 0, NULL, 0, 0),
# (32902, 6094, 0, 0, 'I must build a beacon for this marvelous device!', 9997, 6095, 0, 0, 0, NULL, 0, 55664),
# (-2126848, 8308, 0, 0, 'This Dimensional Imploder sounds dangerous!  How can I make one?', 9994, 0, 0, 0, 0, NULL, 0, 0),
# (35448, 8306, 0, 0, 'I must build a beacon for this marvelous device!', 9997, 8307, 0, 0, 0, NULL, 0, 55664);
#
# DELETE FROM `creature_template_gossip` WHERE `CreatureID`=21494 AND `MenuID`=8306;
# INSERT INTO `creature_template_gossip` (`CreatureID`, `MenuID`, `VerifiedBuild`) VALUES
# (21494, 8306, 55664); -- Smiles O'Byron
#
# DELETE FROM `gossip_menu` WHERE (`MenuID`=6095 AND `TextID`=7252);
# INSERT INTO `gossip_menu` (`MenuID`, `TextID`, `VerifiedBuild`) VALUES
# (6095, 7252, 55664); -- 14743 (Jhordy Lapforge)
#
# UPDATE `gossip_menu_option` SET `GossipOptionID`=35449, `VerifiedBuild`=55664 WHERE (`MenuID`=8306 AND `OptionID`=1);
#
# UPDATE `creature_template` SET `ScriptName`='' WHERE `ScriptName` IN ('npc_prof_blacksmith','npc_prof_leather','npc_prof_tailor','npc_prof_alchemy');
